...
class Presenter:
    def __init__(self) -> None:
        ...
        self.exp_rep: SQLiteRepository[Expense] = \
            SQLiteRepository(db_file="mem.db", cls=Expense)
        self.cat_rep: SQLiteRepository[Category] = \
            SQLiteRepository(db_file="mem.db", cls=Category)
        self.budget_rep: SQLiteRepository[Budget] = \
            SQLiteRepository(db_file="mem.db", cls=Budget)
        ...
        self.window.budget.set_data([str(budget.day),
                                     str(budget.week), str(budget.month)])
        ...
        self.window.add_expenses_widget.del_button.\
            clicked.connect(self.del_handler)
        self.window.add_expenses_widget.add_button.\
            clicked.connect(self.add_handler)
        ...
        if expenses is not None:
            for exp_class in expenses:
                ...
                res.append([exp_class.expense_date,
                            str(exp_class.amount), name, exp_class.comment])
    ...
    def del_handler(self) -> None:
        if self.window.expenses_widget.rowCount() == 0:
            ...
            return
        if self.window.expenses_widget.rowCount() == -1:
    ...
    def add_handler(self) -> None:
        ...
        try:
            ...
            MessageBox('Ошибка', 'Сумма должна быть числом.',
                       is_critical=True, add_cancel=False).exec()
            return

        if amount <= 0:
            MessageBox('Ошибка', 'Сумма должна быть положительной.',
                       is_critical=True, add_cancel=False).exec()
            return

        comment = self.window.add_expenses_widget.edit_comment_box.text()
        date = self.window.add_expenses_widget.edit_date.dateTime().\
            toPython().strftime("%d/%m/%Y, %H:%M:%S")
        num = self.window.add_expenses_widget.\
            categories_list_widget.currentIndex()
        cat_pk = self.cat_rep.get_all()[num].pk
        new_exp = Expense(amount=amount, category=cat_pk,
    ...
    def edit_handler(self) -> None:
        ...
        if self.window.expenses_widget.currentRow() == -1:
            MessageBox('Ошибка', 'Выберите строку для изменения.',
                       is_critical=True, add_cancel=False).exec()
            return
        ...
        num = self.window.add_expenses_widget.\
            categories_list_widget.currentIndex()
        cat_pk = self.cat_rep.get_all()[num].pk
        new_exp = Expense(pk=edit_pk, amount=amount,
                          category=cat_pk, expense_date=date,
                          comment=comment)
    ...
    def add_cat_handler(self) -> None:
        ...
        if self.window.add_expenses_widget.categories_list_widget.\
                findText(txt) != -1:
    ...
    def calc_budget(self) -> None:
        ...
        if exp_data is not None:
            for expense in exp_data:
                exp_date = datetime.strptime(expense.expense_date,
                                             "%d/%m/%Y, %H:%M:%S")
